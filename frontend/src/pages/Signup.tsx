...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || 'Signup failed. Ensure MongoDB is connected.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden font-sans">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-text-muted">Start building your better life today</p>
        </div>
        ...
        <p className="mt-6 text-center text-gray-400">
          ...
          <Link to="/login" className="text-violet hover:text-white transition-colors">
            Log in
          </Link>
        </p>
      </motion.div>
